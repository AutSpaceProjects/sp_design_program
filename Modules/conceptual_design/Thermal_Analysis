...
class ThermalRequirements:
    operational_min_temp_C: float
    operational_max_temp_C: float
    survival_min_temp_C: float
    survival_max_temp_C: float
    heater_power_limit_W: float
...
class TradeStudy:
    ...
    weights: Dict[str, float] = field(default_factory=lambda: {
        "mass": 0.25, "power": 0.25, "reliability": 0.30, "complexity": 0.20
    })
...
class DesignIteration:
    ...
    def run(self, thermal: ThermalSubsystem) -> None:
...
def run_satellite_analysis(
    ...
    candidates: List[ArchitectureCandidate] = None,
    thermal: ThermalSubsystem = None
) -> None:
    ...
    if thermal is None:
        total_heat = (
            ...
            adcs.dissipated_power_W +
            payload.dissipated_power_W +
            structure.dissipated_power_W
        )
        thermal = ThermalSubsystem(
            name="Thermal",
            dissipated_power_W=total_heat,
    ...
    print("\n===== Verification =====\n")
    VerificationModule(thermal=thermal).verify()
